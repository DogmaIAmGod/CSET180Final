...
app = Flask(__name__)
conn_str = "mysql://root:Code123!@localhost/scammazon"
...
def post_get_accounts():
    auth = conn.execute(text(
        "SELECT CASE WHEN EXISTS (SELECT 1 FROM account WHERE username = :username AND (password = :password OR email = :password))THEN 'Yes'ELSE 'No' END"), request.form).one_or_none()
    maybe_user = request.form.get("username")
    print("Auth: ", auth[0])
    if auth[0] == 'Yes':
        type = conn.execute(
            text(f"SELECT type FROM account where username = '{maybe_user}'")).all()[0][0]
        if type == "vendor":
...
def post_product_edit():
    try:
        conn.execute(text("UPDATE products SET {} = :info_change WHERE product_id = :product_id".format(
            request.form.get("type"))), request.form)
        conn.commit()
...
def shopping():
    items = conn.execute(text(
        "SELECT product_id, title, description, image, color, size, price FROM products")).all()
...
def post_shopping():
    person_id = conn.execute(text(
        f"SELECT account_id FROM account where username = '{str(request.cookies.get('logged_in'))}'")).all()[0][0]
    item_id = request.form.get("product_id")
    items = conn.execute(text(
        "SELECT product_id, title, description, image, color, size, price FROM products")).all()
    conn.execute(text(
        f"INSERT INTO cart (`product_id`, `account_id`)VALUES('{item_id}', '{person_id}')"), request.form)
    conn.commit()
    return render_template('Product.html', items=items)
...
def post_add():
    person_id = conn.execute(text(
        f"SELECT account_id FROM account where username = '{str(request.cookies.get('logged_in'))}'")).all()[0][0]
    conn.execute(text(
        f"INSERT INTO products (`account_id`, `title`, `description`, `image`, `color`, `size`, `quantity`, `price`) VALUES ('{person_id}', :title, :description, :image, :color, :size, :quantity, :price)"), request.form)
    conn.commit()
    return redirect("/vendor", code=301)
...
def adminadd():
    vendors = conn.execute(text(
        "SELECT account_id, concat(first_name,' ',last_name) as name from account where type = 'vendor'")).all()
...
def post_cart():
    ...
    total = float(conn.execute(text(
        f"SELECT SUM(price) as total from cart join products using(product_id) where cart.account_id = {person_id}")).all()[0][0])
    order = conn.execute(
        text(f"SELECT product_id FROM cart where account_id = {person_id}")).all()
    order_list = []
    for i in range(len(order)):
        order_list.append(order[i][0])
    item_name = []
    for i in order_list:
        item_name.append(conn.execute(text(
            f"SELECT CONCAT(size,' ',title) from products where product_id = {i}")).all()[0][0].title())
...
def post_delete_cart():
    ...
    cart_id = conn.execute(text(
        f"SELECT cart_id from cart where product_id = {pro_id} AND account_id = {person_id}")).all()[0][0]
    conn.execute(
        text(f"DELETE FROM cart WHERE (cart_id = '{cart_id}')"), request.form)
    conn.commit()
...
def orders():
    ...
    items = eval(conn.execute(
        text(f"SELECT items FROM orders ORDER by order_id DESC LIMIT 1")).all()[0][0])
    item_name = []
    num = 0
    for i in items:
        item_name.append([
            conn.execute(text(
                f"SELECT price, concat(size,' ',title) as item from products where product_id = '{i}'")).all()[num][0],
...
def reviews():
    person_id = conn.execute(text(
        f"SELECT account_id FROM account where username = '{str(request.cookies.get('logged_in'))}'")).all()[0][0]
    orders = conn.execute(text(
        f"SELECT order_status, order_date, text, total FROM orders where account_id = {person_id}")).all()
...
def admin_reviews():
    orders = conn.execute(text(
        f"SELECT order_id, order_status, order_date, text, total FROM orders")).all()
...
def admin_reviews_edit(id=0):
    number = int(id)
    order_edit = conn.execute(text(
        f"SELECT order_id, order_status, order_date, text, total FROM orders WHERE order_id = {number}")).all()
...
def post_admin_reviews_edit():
    conn.execute(text(
        f'UPDATE orders SET order_status = :status WHERE `order_id` = :order_id'), request.form)
    conn.commit()
...
def post_leave_review():
    ...
    conn.execute(text(
        f"INSERT INTO reviews (`order_id`, `rating`, `comment`, `image`, `date`) VALUES (:order_id, :rating, :comment, :image, '{today}')"), request.form)
    conn.commit()
